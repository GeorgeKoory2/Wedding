...
# On Railway/Render the app directory may be read-only; use /tmp for writable storage
...
def init_db():
    with get_db() as db:
        ...
        ''')
        # Add device_id column to existing databases that don't have it
        try:
...
def gallery_delete(media_id):
    # gallery is open; delete is protected by obscurity (secret hold gesture)
    with get_db() as db:
...
def api_gallery():
    # Gallery is open – no auth required
...
def api_my_memories():
    ...
    if not device_id:
        return jsonify({'items': [], 'total': 0, 'page': 1, 'limit': 30})
    try:
        page  = max(1, int(request.args.get('page', 1)))
        limit = max(1, min(100, int(request.args.get('limit', 30))))
    except (ValueError, TypeError):
