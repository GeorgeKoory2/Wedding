...
// File input opened via <label> directly — no JS click needed
fileInput.addEventListener('change', function() {
  // Use a small delay so Android has time to fully populate FileList
  const input = this;
  setTimeout(function() {
    const files = [...input.files];
    if (!files.length) return;
    addFiles(files);
    setTimeout(() => { try { input.value = ''; } catch(e) {} }, 500);
  }, 100);
});
...
function renderQueue() {
  ...
  // Hide drop zone once files are queued so it can't intercept button clicks
  ...
  fileQueue.forEach(item => {
    ...
    if (!isVideo && !item._objUrl) item._objUrl = URL.createObjectURL(item.file);
    const thumbHtml = isVideo
      ? `<div class="queue-thumb-video">🎥</div>`
      : `<img class="queue-thumb" src="${item._objUrl}" alt=""/>`;
...
function removeFile(id) {
  const item = fileQueue.find(q => String(q.id) === id);
  if (item && item._objUrl) { URL.revokeObjectURL(item._objUrl); }
  fileQueue = fileQueue.filter(q => String(q.id) !== id);
  renderQueue();
}

function clearQueue() {
  fileQueue.forEach(q => { if (q._objUrl) URL.revokeObjectURL(q._objUrl); });
...
async function uploadAll() {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      ...
      item.status = 'done';
      if (item._objUrl) { URL.revokeObjectURL(item._objUrl); item._objUrl = null; }
      done++;
    } catch {
      item.status = 'error';
      failed++;
