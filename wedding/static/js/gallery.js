...
window.deleteCurrentItem = async function() {
  ...
  try {
    const urlParams = new URLSearchParams(window.location.search);
    const t = urlParams.get('t') || '';
    const res = await fetch(`/gallery/delete/${id}${t ? '?t=' + encodeURIComponent(t) : ''}`, {
      method: 'POST', credentials: 'same-origin'
    });
    if (!res.ok) throw new Error();

    // Remove ONLY the matching item by ID from both arrays
    const fi = filteredItems.findIndex(i => String(i.id) === String(id));
    const ai = allItems.findIndex(i => String(i.id) === String(id));
    if (fi !== -1) filteredItems.splice(fi, 1);
    if (ai !== -1) allItems.splice(ai, 1);

    // Remove only that one tile from the DOM (no full re-render)
    const tiles = document.querySelectorAll('#galleryGrid .gallery-item');
    if (tiles[fi]) tiles[fi].remove();

    // Update count
    const el = document.getElementById('mediaCount');
    if (el) el.textContent = `${allItems.length} memor${allItems.length !== 1 ? 'ies' : 'y'}`;
    ...
    if (!filteredItems.length) {
      ...
      // Stay on same position, or last item if we deleted the last one
      lbIndex = Math.min(fi !== -1 ? fi : lbIndex, filteredItems.length - 1);
      renderLightbox();
    }
  } catch {
