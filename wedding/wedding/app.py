...
# ── Gallery auth (cookie-based, no session needed) ────────────────────────────
def _make_gallery_token():
    return hmac.new(app.secret_key.encode(), GALLERY_CODE.encode(), hashlib.sha256).hexdigest()


def _gallery_authed():
    token = request.cookies.get('gallery_token', '')
    return hmac.compare_digest(token, _make_gallery_token())
...
def gallery_auth():
    ...
    if code == GALLERY_CODE:
        ...
        resp.set_cookie('gallery_token', _make_gallery_token(),
